...
desktopenv(){
    ...
    # Arch, Manjaro, Artix
        ...
        echo -e "\n${yellow}[+] ${green}Installing ${cyan}$de ${green}desktop environment on ${cyan}$os Linux ${white}"
        pd sh $os --shared-tmp -- /bin/bash -c "pacman -Rns gnome-themes-extra --noconfirm 2&>1"; 
        pd sh $os --shared-tmp -- /bin/bash -c "pacman -Rns gnome-themes-standard --noconfirm 2&>1"; 
    ...
    pd sh $os --shared-tmp -- /bin/bash -c "pacman -Rns gnome-themes-extra --noconfirm 2&>1"; 
    pd sh $os --shared-tmp -- /bin/bash -c "pacman -Rns gnome-themes-standard --noconfirm 2&>1"; 
    for i in gnupg curl nano git xz at-spi2-core gnome gnome-terminal gnome-tweaks gnome-shell-extensions nautilus gedit librsvg inetutils dialog exo dbus gtk-engine-murrine gtk-engines neofetch fish
    do
        echo ""
        echo -e "${yellow}[+] ${green}Installing $i ${white}"
        echo ""
        while ! pd sh $os --shared-tmp -- /bin/bash -c "pacman -S --noconfirm --needed $i"; do
            echo -e "${red}[-] Failed to install $i. Retrying...${white}"
            sleep 2
        done
        echo -e "${green}[+] Successfully installed $i ${white}"
    ...
    echo "for file in \$(find /usr -type f -iname \"*login1*\"); do rm -rf \$file; done" >> x11-start
    echo "export DISPLAY=:1" >> x11-start
    echo "dbus-launch gnome-shell --x11" >> x11-start
    chmod 777 x11-start
    mv x11-start /usr/bin
    chmod 777 /usr/bin/x11-start
    ...
    file_path="/data/data/com.termux/files/usr/bin/$os"
    echo "#! /bin/bash" > $file_path
    echo "" >> $file_path
    echo "termux-x11 :1 &" >> $file_path
    echo "pd sh $os --user $user --shared-tmp " >> $file_path
    chmod 777 $file_path
